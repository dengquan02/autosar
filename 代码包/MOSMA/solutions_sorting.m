...
for i = 1:npop 
    ...
    for j=1:npop
        ...
        if ns_categ_1==0 && ns_categ_2 ~= m % i被j支配
            xSol(i).n=xSol(i).n+1;
        elseif ns_categ_3 == 0 && ns_categ_2 ~= m % i支配j
            xSol(i).q=[xSol(i).q j];
        end
    end % end of j   
    %% Record/Udpate the Pareto Front
    if xSol(i).n==0 % i没有被任何solutions支配，为第一Pareto Front
...
while ~isempty(PF(frontRank).R) % 判断当前frontRank前沿是否空
    nonPF=[];    % Intialization the set
    N=length(PF(frontRank).R);
    for i=1:N
       % Get the solution/list 
       % 当前frontRank第i个solution支配的所有solutions
       ...
       if ~isempty(xSol(Sol_tmp_q))
           for j = 1:length(Sol_tmp_q)
             % Get the solutions dominated by the current solution    
              Sol_tmp_qj=xSol(PF(frontRank).R(i)).q(j);% Sol_tmp_qj=Sol_tmp_q(j);
...
Sorted_frontRank=x(frontRanks_Index,:); % 按frontRank排序
...
% 拥挤度距离的计算方法：将每个非支配层次内的解按照目标函数值排序，然后计算相邻解之间的距离之和作为拥挤度距离。
...
% 先升序，后降序
sorted_x = sortrows(xy, [Rcol, -(Rcol + 1)]);
