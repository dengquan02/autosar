...
function [f, OptiPara] = newMOSMA(dim,M,lb,ub,N,Max_iter,ishow)          
X = zeros(N,dim);
Sol = zeros(N,dim);
...
% OptiPara(1,1) = mean(new_Sol(:,dim + 1))+var(new_Sol(:,dim + 1));
% OptiPara(1,2) = mean(new_Sol(:,dim + 2))+var(new_Sol(:,dim + 2));
% OptiPara(1,1) = median(new_Sol(:,dim + 1));
% OptiPara(1,2) = median(new_Sol(:,dim + 2));
% OptiPara(1,1) = new_Sol(N/2,dim + 1);
% OptiPara(1,2) = new_Sol(N/2,dim + 2);
