...
%         if ~isempty(individual(F(front).f(i)).p)%个体i有自己所支配的解集
        for j = 1 : length(individual(F(front).f(i)).p)%循环个体i所支配解集中的个体
            individual(individual(F(front).f(i)).p(j)).n = individual(individual(F(front).f(i)).p(j)).n - 1;%这里表示个体j的被支配个数减1
...
for front = 1 : (length(F) - 1)%这里减1是因为代码55行这里，F的最后一个元素为空，这样才能跳出循环。所以一共有length-1个排序等级
    ...
    for i = 1 : M
        ...
        % 目标函数归一化
        [xx,~] = mapminmax(sorted_based_on_objective(:,V+i)',0,1);
        sorted_based_on_objective(:,V+i) = xx';
...
%得到的是已经包含等级和拥挤度的种群矩阵 并且已经按等级、拥挤度排序排序
