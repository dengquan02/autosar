...
[~,index] = sort(chromosome(:,V + 1),'ascend'); % 按照cpuload升序排列
% OptiPara(1,1) = sum(chromosome(index(1:pop/2),V + 1))/(pop/2);
% OptiPara(1,2) = sum(chromosome(:,V + 2))/pop;
OptiPara(1,1) = sum(chromosome(:,V + 1))/pop;
OptiPara(1,2) = sum(chromosome(:,V + 2))/pop;
OptiPara(1,3) = sum(chromosome(index(1:pop/2),V + 1))/(pop/2);
...
for i = 1 : gen
    ...
    [~,index] = sort(chromosome(:,V + 1),'ascend');
%     OptiPara(i+1,1) = sum(chromosome(index(1:pop/2),V + 1))/(pop/2);
%     OptiPara(i+1,2) = sum(chromosome(:,V + 2))/pop;
    OptiPara(i+1,1) = sum(chromosome(:,V + 1))/pop;
    OptiPara(i+1,2) = sum(chromosome(:,V + 2))/pop;
    OptiPara(i+1,3) = sum(chromosome(index(1:pop/2),V + 1))/(pop/2);
end
