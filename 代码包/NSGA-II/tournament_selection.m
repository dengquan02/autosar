...
%竞标赛选择法，每次随机选择tour_size（2）个个体，优先选择排序等级高的个体，如果排序等级一样，优选选择拥挤度大的个体
for i = 1 : pool_size
    for j = 1 : tour_size 
        candidate(j) = ceil(pop*rand(1));%随机选择参赛个体
        if candidate(j) == 0
            candidate(j) = 1;
        end
        if j > 1
            while ~isempty(find(candidate(1 : j - 1) == candidate(j)))%防止两个参赛个体是同一个
                candidate(j) = ceil(pop*rand(1));
                if candidate(j) == 0
                    candidate(j) = 1;
                end
            end
    ...
    if length(min_candidate) ~= 1%如果两个参赛者的排序等级相等 则继续比较拥挤度 优先选择拥挤度大的个体
        ...
        f(i,:) = chromosome(candidate(min_candidate),:);
    end
end
